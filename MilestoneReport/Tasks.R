...
cleanSample <- tm::tm_map(cleanSample,
                          tm::content_transformer(function(x)
                              iconv(x, to="UTF-8", sub="byte")))

## Convert to lower case
cleanSample <- tm::tm_map(cleanSample, tm::content_transformer(tolower))

## remove punction, numbers, URLs, stop, profanity and stem wordson
cleanSample <- tm::tm_map(cleanSample, tm::content_transformer(removePunctuation))
cleanSample <- tm::tm_map(cleanSample, tm::content_transformer(removeNumbers))
removeURL <- function(x) gsub("http[[:alnum:]]*", "", x)
cleanSample <- tm::tm_map(cleanSample, tm::content_transformer(removeURL))
cleanSample <- tm::tm_map(cleanSample, stripWhitespace)
cleanSample <- tm::tm_map(cleanSample, removeWords, stopwords("english"))
cleanSample <- tm::tm_map(cleanSample, removeWords, as.vector(profanityWords))
cleanSample <- tm::tm_map(cleanSample, stemDocument)
cleanSample <- tm::tm_map(cleanSample, stripWhitespace)
...
unigram <- ngramTokenizer(finalCorpusDF, 3)
saveRDS(trigram, file = "./MilestoneReport/unigram.RDS")

## unigram plot
unigram <- readRDS("./MilestoneReport/unigram.RDS")
unigramPlot <- gvisColumnChart(trigram, "String", "Count",
